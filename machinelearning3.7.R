...
set.seed(999)
time_series_data1$spprice <- lag(time_series_data1$spprice, k = -12)  # 12 months in a year
na_rows <- apply(is.na(time_series_data1), 1, any)
last_12_na_rows <- tail(which(na_rows), 12)

# Get the corresponding rows for training
training_rows1 <- (last_12_na_rows - 1)  # Adjust if needed

# Create training and testing sets
training_data1 <- time_series_data1[-last_12_na_rows, ]
testing_data1 <- time_series_data1[last_12_na_rows, ]
...
models1 <- list(
  LinearRegression1 = train(spprice ~ ., data = training_data1, method = "lm", trControl = ctrl1),
  RandomForest1 = train(spprice ~ ., data = training_data1, method = "rf", trControl = ctrl1),
  XGBoost1 = train(spprice ~ ., data = training_data1, method = "xgbTree", trControl = ctrl1),
  SVM1 = train(spprice ~ ., data = training_data1, method = "svmRadial", trControl = ctrl1),
  NeuralNetwork1 = train(spprice ~ ., data = training_data1, method = "nnet", trControl = ctrl1))
